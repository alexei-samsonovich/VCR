...
public class GameController : MonoBehaviour
{
    ...
    [SerializeField] private SpeechRecognizerController spc;
    ...
    private void Update()
    {
        ...
        if (rightController.transform.position.y > 2 && isAsking == false && isTalking == true)
        {
            StartCoroutine("isQuestiionInVR");
        }
    ...
    private IEnumerator isQuestiionInVR()
    {
        yield return new WaitForSeconds(1.0f);
        if(rightController.transform.position.y > 2.0f)
        {
            isAsking = true;
            StartCoroutine("askStudentForQuestionDuringLecture");
        }
